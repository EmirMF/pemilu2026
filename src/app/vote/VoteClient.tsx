...
import { Image as ImageIcon, Link as LinkIcon, X, User, UserCircle, Info, AlertTriangle, ArrowLeft } from 'lucide-react';
...
export default function VoteClient({ candidates }: { candidates: Candidate[] }) {
  ...
  return (
    <div>
      {/* Floating Theme Toggle & Back Button */}
      <div className="fixed top-6 left-6 z-50 flex gap-3">
        <button
          onClick={() => router.back()}
          className="p-3 bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-full shadow-lg border border-neutral-200 dark:border-neutral-700 transition-all hover:scale-110"
        >
          <ArrowLeft size={20} />
        </button>
      </div>
          ...
          >
            {c.photo ? (
              ...
              <div className="relative">
                ...
                <span className="absolute bottom-4 left-4 w-8 h-8 rounded-full bg-gradient-to-r from-red-600 to-orange-500 text-white flex items-center justify-center text-sm font-bold">
                  {i + 1}
                </span>
              </div>
            )}
            <h3 className="text-xl font-bold mb-1">{c.name}</h3>
            {c.major && <p className="text-red-500 dark:text-red-400 text-sm mb-2">{c.major}</p>}
            <p className={`text-neutral-600 dark:text-neutral-400 text-sm mb-2 ${expandedId === c.id ? '' : 'line-clamp-3'}`}>{c.vision}</p>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="text-secondary-600 dark:text-secondary-400 text-sm font-medium hover:underline"
            >
            ...
            >
              ...
              <div className="p-6">
                ...
                {selectedModalCandidate.photo ? (
                  <div className="w-full h-100 bg-cream-100 rounded-2xl overflow-hidden mb-6">
                    <img
                ...
                {/* Name */}
                <h4 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50">{selectedModalCandidate.name}</h4>
                {selectedModalCandidate.major && <p className="text-red-500 dark:text-red-400 mb-4">{selectedModalCandidate.major}</p>}
